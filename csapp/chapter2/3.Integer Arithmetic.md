...
$$
x*^{t}_{w}y=U2T_w\left((x\cdot y) \bmod2^w\right)
$$
...
- $K$가 상수일 때 $x * K$를 덧셈, 뺄셈, shift로 optimize하는 경우, $K$를 power of 2로 표현한다. 예를 들어, $14=2^3+2^2+2^1$. 그러면 
$x * 14=x(2^3+2^2+2^1)=x * 2^3+x * 2^2+x * 2^1$
로 표현하게 되고 각각을 shift로 표현하면 $(x \ll 3) + (x \ll 2) + (x \ll 1)$이다. 
- 다음은 위를 일반화한 식이다
