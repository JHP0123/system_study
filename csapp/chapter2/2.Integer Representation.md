...
$$B2U_w(\vec{x})\triangleq\sum_{i=0}^{w-1}x_i2^i$$
...
$$B2T_w(\vec{x})\triangleq-x_{w-1}2^{w-1}+\sum_{i=0}^{w-2}x_i2^i$$
...
$$
B2T_k([x_{k-1}, x_{k-2}, \dots, x_0])=U2T_k\!\left(B2U_w([x_{w-1}, x_{w-2}, \dots, x_0])\bmod 2^k\right)
$$
